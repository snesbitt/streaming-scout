...
**Two live records are currently wrong, and this is the evidence the bug bit again.** The live status store says `Reacher, season 4` and `The Westies` are both `watched`, while `index.html` carries both as static `watching-row` blocks, which is exactly what the 2026-08-16 pass restored them to after establishing that the on-site "watched" button means "caught up for now," not "series finished" (Reacher S4 runs through Sep 16). `scripts/check-status-drift.mjs` will fail on this at its next scheduled run and open an issue, correctly. **The records must be corrected after this change is deployed, not before**. Correcting them now would write through the buggy code that lost them in the first place. Baseline captured before the change, for post-deploy comparison: 6 status entries, 16 dismissals (full JSON in the Streaming Guide project doc).
