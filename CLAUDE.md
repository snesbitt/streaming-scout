...
## 2026-08-19 - Currently Watching rows now say which day the next episode drops

Susan's ask, with an explicit "do not botch the CSS." No CSS was touched and no
markup structure changed: the day went inside each row's existing
`.watching-meta` span, so `.watching-row` / `.watching-info` / `.watching-badge`
render exactly as before. Rows that gained words gain a wrapped line at 390px;
nothing else moved. Verified by headless render at 390px and 1100px, before and
after, comparing the Currently Watching card directly.

Every day was researched against sources rather than inferred, and two shows
turned out to have no next episode at all:
...
`check-content-drift.mjs` exist to catch elsewhere. Three of them go stale on a
known date (Westies Aug 23, Hardacres Sep 9, Reacher Sep 16), and a rebuild that
regenerates Currently Watching from the Cowork skill will drop the days entirely
